...
- Source wall clock Syslog интерпретируется в IANA timezone устройства. Timestamp
  исходного CDR SMG уже является UTC и не получает повторного локального смещения.
  Canonical UTC используется для matching, `received_at` остаётся отдельным фактом.
