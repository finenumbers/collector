...
UI «Регион A/B» stores territory). Only **11-digit** numbers starting with 73/74/78/79
(or trunk-8 forms 83/84/88/89, including 7800/8800) are looked up. Bare 10-digit
nationals are never sent to PSTN; those sides store `-`. Eligible phones missing
from the PSTN database store `Не существует` (not counted as lookup errors). Both
operator and territory are
