...
- После bump `SyslogParserVersion` (например `smg-3.410-v8`): дождитесь лога
  ...
  должен опустеть (`cdr`/`alarm-led`/`SNMP`/`last message repeated` уходят в
  `system_journal`/`alarms`).
