...
func (c *Client) listCurrentEventsPage(
	...
	query := `WITH page AS
		(
			SELECT event_id,
				argMax(received_at,interpreted_at) AS received_at,
			...
			FROM collector.syslog_facts AS sf
			WHERE sf.device_id=? AND sf.timezone_revision=?`
	...
	if search != "" {
		query += ` AND sf.event_id IN
			(SELECT event_id FROM collector.raw_syslog
			 WHERE device_id=? AND positionCaseInsensitive(payload,?)>0)`
		args = append(args, deviceID, search)
	}
	if cursor != nil {
		query += ` AND (sf.received_at<? OR (sf.received_at=? AND sf.event_id<?))`
	...
	query += ` GROUP BY event_id
		...
		SELECT p.event_id,p.received_at,p.event_time,p.category,p.component,p.message,
			r.payload,p.parse_status,p.attributes,p.source_timezone
		FROM page AS p
		ANY INNER JOIN collector.raw_syslog AS r
			ON r.device_id=? AND r.event_id=p.event_id
...
func (c *Client) listFallbackEventsPage(
	...
	query := `SELECT event_id,max(received_at) AS max_received_at,
		argMax(event_time,received_at),argMax(category,received_at),
		argMax(component,received_at),argMax(message,received_at),
		argMax(payload,received_at),argMax(parse_status,received_at),
		argMax(attributes,received_at),argMax(source_timezone,received_at)
	...
	query += ` GROUP BY event_id ORDER BY max_received_at DESC,event_id DESC LIMIT ?`
...
func (c *Client) listCurrentCallsPage(
	...
	query := `WITH times AS
		...
		page AS
		(
			SELECT c.record_id AS page_record_id,t.setup_time,
				coalesce(t.setup_time,c.ingested_at) AS sort_time
			FROM times AS t
	...
	query += ` ORDER BY sort_time DESC,c.record_id DESC LIMIT ?
		...
		SELECT c.record_id,p.setup_time,c.duration_ms,c.release_cause,c.release_info,
...
func (c *Client) listCurrentAntifraudPage(
	...
	query := `WITH page AS
		(
			SELECT transaction_id,
				...
				argMax(acct_session_id,updated_at) AS acct_session_id,
				argMax(request_type,updated_at) AS request_type,
				argMax(request_code,updated_at) AS request_code,
				argMax(response_code,updated_at) AS response_code,
				argMax(decision,updated_at) AS decision,
				argMax(decision_reason,updated_at) AS decision_reason,
				argMax(server_address,updated_at) AS server_address,
				argMax(retries,updated_at) AS retries,
				argMax(latency_ms,updated_at) AS latency_ms,
				argMax(calling_station_id,updated_at) AS calling_station_id,
				argMax(called_station_id,updated_at) AS called_station_id,
				argMax(src_number_in,updated_at) AS src_number_in,
				argMax(dst_number_in,updated_at) AS dst_number_in,
				argMax(src_number_out,updated_at) AS src_number_out,
				argMax(dst_number_out,updated_at) AS dst_number_out,
				argMax(in_trunkgroup_label,updated_at) AS in_trunkgroup_label,
				argMax(out_trunkgroup_label,updated_at) AS out_trunkgroup_label,
				argMax(accounting_status,updated_at) AS accounting_status,
				argMax(q850_cause,updated_at) AS q850_cause,
				argMax(completeness,updated_at) AS completeness,
				argMax(attributes,updated_at) AS attributes
	...
	query += ` GROUP BY transaction_id
			ORDER BY last_event_at DESC,transaction_id DESC LIMIT ?
		),
		assignments AS
		(
			SELECT transaction_id,
		...
		(
			...
			WHERE device_id=? AND timezone_revision=?
			  AND record_id IN (SELECT assumeNotNull(cdr_record_id) FROM assignments
				WHERE cdr_record_id IS NOT NULL)
			GROUP BY record_id
		)
		SELECT p.transaction_id,p.first_event_at,p.last_event_at,p.call_context,
			...
			if(a.cdr_record_id IS NULL,[],[assumeNotNull(a.cdr_record_id)]) AS record_ids,
			toUInt64(if(a.cdr_record_id IS NULL,0,1)) AS leg_count,ct.setup_time,
...
func (c *Client) currentDiagnostics(
	...
	_ = c.Conn.QueryRow(ctx, `SELECT revision,timezone,status,processed,raw_total,
		...
		FROM collector.device_derived_revisions FINAL
		WHERE device_id=? AND status='building'
