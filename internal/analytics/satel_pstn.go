...
// EnrichSatelRecords looks up unique bill_ani/bill_dnis and fills operator/region.
// Missing token, empty phones, or lookup failures leave fields empty; never fails ingest.
func EnrichSatelRecords(ctx context.Context, client *pstnlookup.Client, records []SatelRTURecord) {
	if client == nil || !client.Enabled() || len(records) == 0 {
		return
	}
	...
	byPhone := client.LookupMany(ctx, phones, 8)
...
// ListSatelRTURecordsNeedingPSTNEnrichment pages FINAL rows with phones but empty
// enrichment fields, ordered by record_id for stable cursor pagination.
func (c *Client) ListSatelRTURecordsNeedingPSTNEnrichment(
	...
	defer release()
	query := `SELECT record_id,device_id,file_id,row_number,ingested_at,parser_version,template_key,cdr_id,
		...
		WHERE (bill_ani!='' OR bill_dnis!='')
			AND bill_ani_operator='' AND bill_dnis_operator=''
