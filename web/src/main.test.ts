...
describe('custom antifraud UI cleanup', () => {
  ...
  it('exposes runtime settings editor in system settings', () => {
    ...
    expect(main).toContain('>Сохранение данных</button>')
    expect(main).toContain('DataSaveSettingsPage')
    expect(main).toContain('/system/syslog-archive/status')
    expect(main).toContain('/system/syslog-archive/jobs?')
    expect(main).toContain('archive-jobs-shell')
    expect(main).toContain('Нет задач архива за последние сутки')
    expect(main).toContain('Проверить каталог')
    expect(main).toContain('Syslog archive · worker / failed / lag')
